...
from pipelines.generation_evaluation_pipeline import generation_evaluation_pipeline
...
def cmd_eval_generation(args):
    """Run generation evaluation using LLM-as-a-Judge."""
    print("🧪 Clinical RAG - Generation Evaluation (LLM-as-a-Judge)")
    print("=" * 60)
    
    try:
        print(f"🔍 Running generation evaluation with parameters:")
        print(f"  Dataset: {args.dataset}")
        ...
        # Run generation evaluation pipeline
        pipeline_run = generation_evaluation_pipeline(
            dataset_path=args.dataset,
            ...
            temperature=args.temperature,
            save_detailed_report=not args.no_save,
            report_output_dir="evaluation/reports/generation"
        )
        
        # Get the final report
        final_report = pipeline_run.steps["generate_generation_report"].output.load()
        ...
        if final_report['success']:
            ...
            print(f"📊 Results saved as ZenML metadata")
            print(f"\n🎯 Key Metrics:")
            print(f"  Overall Score: {summary['overall_score']} ({summary['overall_percentage']})")
            ...
            for dimension in ['clinical_accuracy', 'relevance', 'evidence_support']:
                ...
                percentage_key = f'{dimension}_percentage'
                if score_key in summary:
                    print(f"  {dimension.replace('_', ' ').title()}: {summary[score_key]} ({summary[percentage_key]})")
            ...
            print(f"❌ Generation evaluation failed: {final_report['error']}")
            sys.exit(1)
            
    except Exception as e:
        print(f"❌ Generation evaluation pipeline error: {e}")
        sys.exit(1)
...
Examples:
  ...
  # Run generation evaluation with LLM-as-a-Judge
  ...
  python run.py status
    ...
    # Generation Eval command
    eval_gen_parser = subparsers.add_parser('eval-generation', help='Run generation evaluation using LLM-as-a-Judge')
    eval_gen_parser.add_argument('--dataset', default='evaluation/test_datasets/clinical_qa_pairs.json', 
                                help='Path to evaluation dataset (default: evaluation/test_datasets/clinical_qa_pairs.json)')
    eval_gen_parser.add_argument('--generation-model', default='ollama/llama3.2:3b', 
                                help='Model for generating responses (default: ollama/llama3.2:3b)')
    eval_gen_parser.add_argument('--judge-model', default='openai/gpt-3.5-turbo', 
                                help='Model for LLM-as-a-Judge evaluation (default: openai/gpt-3.5-turbo)')
    eval_gen_parser.add_argument('--top-k', type=int, default=5, 
                                help='Number of top results to retrieve for generation (default: 5)')
    eval_gen_parser.add_argument('--threshold', type=float, default=0.3, 
                                help='Similarity threshold for retrieval (default: 0.3)')
    eval_gen_parser.add_argument('--no-enhance', action='store_true', 
                                help='Disable query enhancement')
    eval_gen_parser.add_argument('--temperature', type=float, default=0.1, 
                                help='Generation temperature (default: 0.1)')
    eval_gen_parser.add_argument('--no-save', action='store_true', 
                                help='Do not save detailed reports to disk')
    eval_gen_parser.set_defaults(func=cmd_eval_generation)
